...
最終更新: v0.12.0 時点。新しいAIセッションを開始する際は、このファイルを読ませること。
...
- `src/PluginEditor.h` 内 PresetPanel — **PRESETSタブ(v0.12.0)**: 全APVTS状態を `<userAppData>/VoxMorph/Presets/名前.vmpreset`(XML)に保存。プルダウン選択=即読込、Save(同名上書き・空欄なら選択中に上書き)、Delete(確認あり)、Reset All to Defaults。ファイルベースなのでスタンドアロン/AU/VST3で共通
