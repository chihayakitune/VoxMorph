...
最終更新: v0.7.0 時点。新しいAIセッションを開始する際は、このファイルを読ませること。
...
- `dsp/PsolaEngine.h` — **依存ゼロの単一ヘッダC++17**。エンジン本体(TD-PSOLA):
  ...
  - **GCI Grain Sync(v0.7.0、gciトグル・既定オフ)**: グレイン整列を従来のエネルギーピーク探索から、声帯閉鎖瞬間(微分エネルギー最大点=各周期の最急峻フランク、中央差分でHFノイズ耐性)+周期間トラッキング(lastGci+k*P近傍を優先、自由探索ピークが2倍強い時だけ乗換=ダブルパルス飛び移り防止)に切替。息/無声/平坦区間は自動でalignToPeakへフォールバック、オフ時は完全従来動作。検証: クリーン母音は完全一致(非破壊)、不規則フライ声+7stで出力周期性0.066→0.225(3.4倍)
