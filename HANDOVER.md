...
最終更新: v0.6.1 時点。新しいAIセッションを開始する際は、このファイルを読ませること。
...
- `dsp/PsolaEngine.h` — **依存ゼロの単一ヘッダC++17**。エンジン本体(TD-PSOLA):
  ...
  - **Air Preserve(Mixed区間処理、v0.6.0/強化v0.6.1)**: 因果2タップピッチコム(P, 2P遅延・Catmull-Rom補間)で周期成分を予測→残差をairband(既定1kHz)ハイパス=息成分。グレインは息を除いた `harmBuf` から切出し、息は**ピッチ変換せず**遅延整列して出力に加算(連続ノイズのまま)。ノブ0〜0.7=分離量0〜最大(2/3=分散最適、これ超の減算は逆にノイズ増)でエネルギー中立、0.7〜1=息を追加ブースト(最大約+4dB、可聴性・マスキング用)。有声ゲート+5msデジッパー、ノブ0で完全従来動作+自動スキップ
...
pitch, formant, consonant, f1shift/f1gain/f2shift/f2gain/f3shift/f3gain, range(抑揚)/center, breath2(Beta), air/airband(Air Preserve), tilt, jitter, robot/robotHz, lowvoice, lowlat, pitchfloor, automute, mix, gain
