...
最終更新: v0.14.0 時点。新しいAIセッションを開始する際は、このファイルを読ませること。
...
- **外部FXホスティング(v0.14.0、スタンドアロン専用)**: JUCE_PLUGINHOST_VST3=1(**アンダースコア位置に注意、JUCE_PLUGIN_HOST_ではない**)。FxBar(タブ上部の帯、standaloneのみ表示)でPre FX(モノ化直後=変換前、RXノイズ除去等の用途)とPost FX(変換後出力)のVST3を各1つ読込。processor::loadFx/clearFx/getFx、音声スレッドはScopedTryLockでスワップ中1ブロックスキップ。モノ信号は複製→ステレオ処理→平均(applyFxMono)。制限: FXレイテンシ補正なし、再起動後は再読込必要(状態保存は未実装)。UI方針: **共通機能=タブ、スタンドアロン専用=上部バー**。VST2はSteinbergライセンス終了のため対応不可(ユーザー了承済み)
