...
int main (int argc, char** argv)
{
    if (argc < 6)
    {
        std::fprintf (stderr, "usage: %s <in.wav> <out.wav> <pitchSt> <formantSt> <disperse> [smooth] [body] [onsetHold] [holdTracksPeriod] [preLockLowCut] [backfill] [hostBuf] [spec] [air] [relRepair] [relShelf] [cutMode]\n", argv[0]);
        return 2;
    }
    ...
    p.releaseCutMode= argc > 17 ? std::atoi (argv[17]) : 0;
